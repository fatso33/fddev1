...
export class StudioBridgeAdapter {
  constructor(studioState, { diagnostics = null } = {}) {
  ...
  ingestTelemetry(flatMap) {
    this.diagnostics?.count('studio.telemetry.framesReceived');
    this.diagnostics?.enabled && this.diagnostics.count('studio.telemetry.fieldsReceived', Object.keys(flatMap || {}).length);
