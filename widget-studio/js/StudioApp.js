...
import { DiagnosticsAccumulator, diagnosticsEnabled } from '../core/diagnostics/Accumulator.js';
...
export class StudioApp {
  constructor(rootContainer) {
    ...
    const diagnostics = new DiagnosticsAccumulator({
      enabled: diagnosticsEnabled(new URLSearchParams(window.location.search).get('bindingV2Diagnostics')),
      context: { process: 'studio', mode: 'opt-in-query' }
    });
    window.flightDeckBindingV2Diagnostics = diagnostics;
    this.state = new StudioState({ diagnostics });
    ...
    this.bridgeAdapter = new StudioBridgeAdapter(this.state, { diagnostics });
