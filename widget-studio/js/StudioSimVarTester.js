...
export class StudioSimVarTester {
  ...
  constructor(container, state, simBridge) {
    ...
    this.isOpen = false;
    this._diagnosticActiveTests = new Set();
    this._attachDiagnosticLifecycle();
    this.initDOM();
    this.attach();
  }
  ...
  close() {
    if (this._diagnosticActiveTests.size) {
      this.state.diagnostics?.count('studio.tests.drawerClosedWhilePending');
      this.state.diagnostics?.sample('studio.tests.activeCount', this._diagnosticActiveTests.size);
    }
    this.isOpen = false;
  ...
  handleParse() {
    ...
    if (parsed.kind === 'read') {
      ...
      // Wave 1C fix: keep the "K:" prefix. Fire & Watch's event field feeds
      // sendEvent() -> the Bridge's dispatchSimEvent(), which distinguishes
      // an explicit raw address (A:/L:/H:/K:-prefixed) from a logical Deck
      // Event name to resolve through the active profile purely by whether
      // that prefix is present. Stripping it here — as this line used to —
      // turned a raw K:Event into an indistinguishable bare name, which then
      // (mis)resolved as a logical Deck Event lookup instead of addressing
      // the intended raw K target, silently firing the wrong event or
      // nothing at all. A bare name typed directly into this field by hand
      // is still a logical Deck Event and still resolves through the
      // profile, unaffected by this change.
      this.container.querySelector('#svt-fw-event').value = /^K:/i.test(parsed.event) ? parsed.event : `K:${parsed.event}`;
      if (parsed.value !== null) this.container.querySelector('#svt-fw-value').value = parsed.value;
    } else {
  ...
  async handleTest() {
    ...
    const diagnosticOperation = this._diagnosticStart('probe-read');
    try {
      const value = await this.simBridge.probeReadSimVar(parsed.name, parsed.unit || '');
      this.state.diagnostics?.count('studio.tests.observations');
      resultEl.textContent = `✅ Live value: ${value}`;
    } catch (err) {
      resultEl.textContent = `❌ ${err.message}`;
    } finally {
      this._diagnosticEnd(diagnosticOperation);
    }
  }
  ...
  async handleWiggle() {
    ...
    if (!this.wiggleSearching) {
      ...
      listEl.innerHTML = '';
      const diagnosticOperation = this._diagnosticStart('wiggle');
      this._diagnosticWiggle = diagnosticOperation;
      try {
    ...
    try {
      ...
      this._diagnosticEnd(this._diagnosticWiggle);
  ...
  async handleFireAndWatch() {
    ...
    btn.disabled = true;
    const diagnosticOperation = this._diagnosticStart('fire-and-watch');
    try {
  ...
  _attachDiagnosticLifecycle() {
    if (!this.state.diagnostics?.enabled || !this.simBridge?.eventBus?.subscribe) return;
    // Observers reconcile diagnostic records only; they do not cancel RPCs.
    this.simBridge.eventBus.subscribe('BRIDGE_STATUS', ({ connected }) => {
      if (connected || !this._diagnosticActiveTests.size) return;
      this.state.diagnostics.count('studio.tests.outcomeUnknownAfterDisconnect', this._diagnosticActiveTests.size);
      this._diagnosticActiveTests.clear();
      this.state.diagnostics.sample('studio.tests.activeCount', 0);
    });
    this.simBridge.eventBus.subscribe('BINDING_V2_WIGGLE_EXPIRED', () => {
      if (!this.wiggleSearching || !this._diagnosticActiveTests.has(this._diagnosticWiggle)) return;
      this.state.diagnostics.count('studio.tests.wiggleExpiryObserved');
      this._diagnosticEnd(this._diagnosticWiggle);
    });
  }

  _diagnosticStart(name) {
    if (!this.state.diagnostics?.enabled) return null;
    const operation = Symbol(name);
    this._diagnosticActiveTests.add(operation);
    this.state.diagnostics?.sample('studio.tests.activeCount', this._diagnosticActiveTests.size);
    return operation;
  }

  _diagnosticEnd(name) {
    if (!this._diagnosticActiveTests.delete(name)) return;
    this.state.diagnostics?.sample('studio.tests.activeCount', this._diagnosticActiveTests.size);
  }
}
