...
export class SimBridge {
  ...
  connect() {
    ...
    try {
      ...
      this.ws.onmessage = (event) => {
        try {
          const finishParse = this.eventBus.diagnostics?.begin();
          const packet = JSON.parse(event.data);
          finishParse?.('client.websocket.jsonParseMs');
  ...
  async handleIncomingMessage(packet) {
    if (!packet) return;
    if (packet.type === 'WIGGLE_EXPIRED' && this.eventBus.diagnostics?.enabled) {
      this.eventBus.publish('BINDING_V2_WIGGLE_EXPIRED', {});
    }
  ...
  sendRaw(payload) {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      ...
      const diagnostics = this.eventBus.diagnostics;
      if (diagnostics?.enabled) {
        diagnostics.count('client.websocket.localSubmissions');
        diagnostics.count('client.websocket.submittedBytes', new TextEncoder().encode(serialized).byteLength);
        diagnostics.sample('client.websocket.bufferedAmountAfterSendBytes', this.ws.bufferedAmount || 0);
        if (payload.type === 'event') diagnostics.count('client.commands.localSubmissions');
      }
      return true;
    }
    return false;
